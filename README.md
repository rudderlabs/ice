...
## Basic setup: 
Using basic setup, you don't need any extra code change and you will use the provided bootstrap.groovy. You will need to construct your own ice.properties file and have it in your classpath (I put it at src/java). You can use sample.properties (located at src/java) file as the template. Tag key/value consolidation, kubernetes data reports, and account configuration can be specified in a json or yaml file placed in the billing bucket using the name ice_config.json or ice_config.yaml. Use ice_config.sample (located at src/java) as a template.

The processor will by default pull account information from the AWS Organizations service and does not need to be defined in the ice.properties file. Additional account-specific configuration can also be set using Orgranizations Resource Tags:

	| Organizations Tag Key | ice_config Attribute | ice.properties Name                |
	| --------------------- | -------------------- | ---------------------------------- |
	| IceName               | name                 | ice.account.<name>                 |
	| IceRiProducts         | riProducts           | ice.owneraccount.<name>            |
	| IceRole               | role                 | ice.owneraccount.<name>.role       |
	| IceExternalId         | externalId           | ice.owneraccount.<name>.externalId |
	
Default tagging of untagged resources in an account can be specified with Organizations Tags or in the ice_config file. Default tagging values cannot be specified in ice.properties.
...
2. Reader configuration
  ...
  2.5 Specify your organization name in ice.properties. This will show up in the UI header.

          ice.companyName=Your Company Name

  2.6 You can choose to show cost in currency other than "$". To enable other currency, specify the following properties in ice.properties:
  ...
  2.7 By default, Ice pulls in [Highstock](http://www.highcharts.com/) from its CDN.
